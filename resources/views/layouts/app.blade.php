...
<head>
    ...
    <title>{{ config('app.name', 'Severus Cues') }} - {{ __('app.brand_tagline') }}</title>
    ...
    <div class="min-h-screen flex flex-col justify-between relative overflow-hidden">
        ...
        <footer class="relative z-10 bg-[#050806] border-t border-[#1f2e24] py-12 px-4 sm:px-6 lg:px-8 mt-20">
            <div class="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8">
                ...
                <div class="md:col-span-2">
                    <div class="flex items-center space-x-3 mb-4">
                        <img src="{{ asset('images/logo.png') }}" alt="Severus Cues Logo" class="h-10 w-auto object-contain rounded filter drop-shadow-[0_0_8px_rgba(0,230,118,0.5)]">
                ...
                <div>
                    ...
                    <ul class="space-y-2 text-sm text-slate-300">
                        <li><a href="#cues" class="hover:text-[#00e676] transition-colors">{{ __('app.nav.cues') }}</a></li>
                        <li><a href="#chalk" class="hover:text-[#00e676] transition-colors">{{ __('app.nav.chalk') }}</a></li>
                        <li><a href="#accessories" class="hover:text-[#00e676] transition-colors">{{ __('app.nav.accessories') }}</a></li>
                        <li><a href="#technology" class="hover:text-[#00e676] transition-colors">{{ __('app.nav.technology') }}</a></li>
                    </ul>
                </div>
                ...
                <div>
                    ...
                    <div class="space-y-3">
                        <div class="flex items-center space-x-2">
                            <a href="{{ route('lang.switch', 'en') }}" class="px-3 py-1 text-xs rounded font-bold border transition-colors {{ app()->getLocale() === 'en' ? 'bg-[#00e676] text-black border-[#00e676]' : 'border-[#1f2e24] text-slate-400 hover:text-white' }}">EN</a>
                            <a href="{{ route('lang.switch', 'id') }}" class="px-3 py-1 text-xs rounded font-bold border transition-colors {{ app()->getLocale() === 'id' ? 'bg-[#00e676] text-black border-[#00e676]' : 'border-[#1f2e24] text-slate-400 hover:text-white' }}">ID</a>
                        </div>
                        <div>
                            @auth
                                <a href="{{ route('admin.dashboard') }}" class="text-xs text-[#00e676] hover:underline flex items-center">
                            ...
                            @else
                                <a href="{{ route('admin.login') }}" class="text-xs text-slate-400 hover:text-white flex items-center transition-colors">
            ...
            <div class="max-w-7xl mx-auto mt-10 pt-6 border-t border-[#1f2e24] flex flex-col md:flex-row justify-between items-center text-xs text-slate-500">
                <p>&copy; {{ date('Y') }} Severus Cues. {{ __('app.footer.rights') }}</p>
