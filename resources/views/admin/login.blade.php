...
<div class="min-h-screen pt-28 pb-16 flex items-center justify-center px-4">
    <div class="max-w-md w-full venom-card p-8 rounded-2xl border border-[#00e676]/40 shadow-[0_0_30px_rgba(0,230,118,0.2)]">
        <div class="text-center mb-8">
            <img src="{{ asset('images/logo.png') }}" alt="Severus Logo" class="h-16 w-auto mx-auto object-contain filter drop-shadow-[0_0_10px_rgba(0,230,118,0.8)]">
            <h2 class="text-2xl font-black text-white mt-4 uppercase tracking-wider">{{ __('app.admin.title') }}</h2>
        ...
        <form action="{{ route('admin.login.submit') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input type="email" name="email" value="{{ old('email', 'admin@severus.com') }}" required class="w-full px-4 py-3 rounded-xl bg-[#0a0f0d] border border-[#1f2e24] focus:border-[#00e676] text-white text-sm outline-none transition-colors">
            </div>
