    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 reaper-glass-card p-6 rounded-2xl">
        ...
        <div class="flex items-center space-x-3">
            ...
            <form action="{{ route('admin.logout') }}" method="POST">
                @csrf
                <button type="submit" class="px-4 py-2.5 rounded-xl bg-[#121a15] hover:bg-red-500/20 border border-[#1f2e24] hover:border-red-500/40 text-slate-300 hover:text-red-400 font-bold text-xs uppercase transition-colors min-h-[44px]">
                    {{ __('app.admin.logout') }}
                </button>
            </form>
        </div>
