...
<header x-data="{ scrolled: false, mobileMenuOpen: false }" 
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between">
            ...
            <nav class="hidden md:flex items-center space-x-8 text-xs font-bold uppercase tracking-wider text-slate-300">
                ...
                <a href="#accessories" @click.prevent="$dispatch('scroll-to', 'accessories')" class="hover:text-[#00E676] transition-colors py-1 relative group cursor-pointer">
                    {{ __('app.nav.accessories') }}
                    <span class="absolute bottom-0 left-0 w-0 h-0.5 bg-[#00E676] group-hover:w-full transition-all duration-300"></span>
                </a>
            ...
            <div class="flex items-center space-x-3 md:hidden">
                <div class="flex items-center bg-[#141D17] p-0.5 rounded-lg border border-[#00E676]/20 text-[10px]">
                    <a href="{{ route('lang.switch', 'en') }}" class="px-2 py-0.5 rounded font-bold {{ app()->getLocale() === 'en' ? 'bg-[#00E676] text-black' : 'text-slate-400' }}">EN</a>
                    <a href="{{ route('lang.switch', 'id') }}" class="px-2 py-0.5 rounded font-bold {{ app()->getLocale() === 'id' ? 'bg-[#00E676] text-black' : 'text-slate-400' }}">ID</a>
                </div>
    ...
    <div x-show="mobileMenuOpen" 
        ...
        <nav class="flex flex-col space-y-3 font-outfit text-sm font-bold uppercase tracking-wider text-slate-200">
            ...
            <a @click.prevent="mobileMenuOpen = false; $dispatch('scroll-to', 'accessories')" href="#accessories" class="py-2.5 px-4 rounded-xl hover:bg-[#00E676]/10 hover:text-[#00E676] transition-all flex items-center justify-between cursor-pointer">
                <span>{{ __('app.nav.accessories') }}</span>
                <svg class="w-4 h-4 text-[#00E676]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
            </a>
...
class="space-y-24 pt-28">
    ...
    <!-- 2. Product Catalog Section (Cues) -->
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 reveal-on-scroll">
            <div>
                <h2 class="text-3xl sm:text-4xl font-black text-white uppercase font-outfit tracking-tight">
                    {{ __('app.catalog.title_prefix') }} <span class="text-[#00E676]">{{ __('app.catalog.title_highlight') }}</span>
                </h2>
            ...
            <div class="flex flex-wrap items-center gap-2 bg-[#141D17] p-1.5 rounded-2xl border border-[#00E676]/20">
                ...
                @foreach($categories as $category)
                    <button @click="activeCategory = '{{ $category->slug }}'" :class="activeCategory === '{{ $category->slug }}' ? 'bg-[#00E676] text-black font-extrabold shadow-[0_0_15px_rgba(0,230,118,0.5)]' : 'text-slate-400 hover:text-white font-bold'" class="px-4 py-2 rounded-xl text-xs uppercase tracking-wider transition-all">
                        {{ $category->name }}
                    </button>
                @endforeach
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            @foreach($products as $index => $product)
                <div x-show="activeCategory === 'all' || activeCategory === '{{ $product->category->slug }}'" 
                     ...
                     class="venom-card rounded-3xl p-6 flex flex-col justify-between group relative overflow-hidden reveal-on-scroll delay-{{ ($index % 3 + 1) * 100 }}">
                    ...
                    <div class="mt-6 pt-4 border-t border-[#1f2e24] flex items-center justify-between">
                        ...
                        <div class="flex items-center space-x-2">
                            <!-- View Specs Button -->
                            <button @click="activeModalProduct = {{ $product->toJson() }}" class="p-2.5 rounded-xl bg-[#141D17] hover:bg-[#00E676]/20 border border-[#00E676]/30 text-[#00E676] transition-colors" title="{{ __('app.catalog.view_specs') }}">
    ...
    <div id="accessories" class="scroll-mt-28"></div>
        ...
        <div class="venom-card p-8 sm:p-12 rounded-3xl relative overflow-hidden reveal-on-scroll">
            ...
            <div class="max-w-3xl space-y-6">
                ...
                <h2 class="text-3xl sm:text-5xl font-black text-white font-outfit uppercase tracking-tight">
                    {{ __('app.tech.title_prefix') }} <span class="text-[#00E676]">{{ __('app.tech.title_highlight') }}</span>
                </h2>
