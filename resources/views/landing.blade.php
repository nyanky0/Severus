        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-20">
                ...
                <a href="{{ route('landing') }}" class="flex items-center space-x-3 group">
                    <img src="{{ asset('images/logo.png') }}" alt="Severus Cues Logo" class="h-10 w-auto object-contain transition-transform group-hover:scale-110 filter drop-shadow-[0_0_10px_rgba(0,230,118,0.6)]">
                ...
                <nav class="hidden md:flex items-center space-x-8 text-sm font-medium">
                    <a href="#home" class="text-slate-200 hover:text-[#00e676] transition-colors py-1 border-b-2 border-transparent hover:border-[#00e676]">{{ __('app.nav.home') }}</a>
                    <a href="#cues" class="text-slate-200 hover:text-[#00e676] transition-colors py-1 border-b-2 border-transparent hover:border-[#00e676]">{{ __('app.nav.cues') }}</a>
                    <a href="#chalk" class="text-slate-200 hover:text-[#00e676] transition-colors py-1 border-b-2 border-transparent hover:border-[#00e676]">{{ __('app.nav.chalk') }}</a>
                    <a href="#accessories" class="text-slate-200 hover:text-[#00e676] transition-colors py-1 border-b-2 border-transparent hover:border-[#00e676]">{{ __('app.nav.accessories') }}</a>
                    <a href="#technology" class="text-slate-200 hover:text-[#00e676] transition-colors py-1 border-b-2 border-transparent hover:border-[#00e676]">{{ __('app.nav.technology') }}</a>
                </nav>
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <div class="flex items-center bg-[#121a15] border border-[#1f2e24] rounded-lg p-1">
                        <a href="{{ route('lang.switch', 'en') }}" class="px-2.5 py-1 text-xs font-bold rounded {{ app()->getLocale() === 'en' ? 'bg-[#00e676] text-black shadow-[0_0_10px_rgba(0,230,118,0.5)]' : 'text-slate-400 hover:text-white' }}">EN</a>
                        <a href="{{ route('lang.switch', 'id') }}" class="px-2.5 py-1 text-xs font-bold rounded {{ app()->getLocale() === 'id' ? 'bg-[#00e676] text-black shadow-[0_0_10px_rgba(0,230,118,0.5)]' : 'text-slate-400 hover:text-white' }}">ID</a>
                    </div>

                    <!-- Team Portal Access -->
                    @auth
                        <a href="{{ route('admin.dashboard') }}" class="p-2 rounded-lg bg-[#00e676]/20 border border-[#00e676]/50 text-[#00e676] hover:bg-[#00e676]/30 transition-colors" title="Admin Dashboard">
                    ...
                    @else
                        <a href="{{ route('admin.login') }}" class="p-2 rounded-lg bg-[#121a15] border border-[#1f2e24] text-slate-400 hover:text-white transition-colors" title="Team Login">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            <div class="lg:col-span-5 relative flex justify-center items-center">
                <div class="relative w-full max-w-md aspect-square rounded-3xl p-8 bg-gradient-to-b from-[#121a15]/80 to-[#0a0f0d]/90 border border-[#00e676]/30 shadow-[0_0_50px_rgba(0,230,118,0.2)] backdrop-blur-xl flex flex-col justify-center items-center group">
                    ...
                    <img src="{{ asset('images/logo.png') }}" alt="Severus Snake Logo" class="w-64 h-64 object-contain filter drop-shadow-[0_0_20px_rgba(0,230,118,0.8)] animate-float">
    ...
    <section class="py-12 bg-gradient-to-r from-[#06180e] via-[#0a1f13] to-[#06180e] border-y border-[#1f2e24]">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <a href="https://www.tokopedia.com/severus" target="_blank" class="px-6 py-3 rounded-xl bg-[#42b549] hover:bg-[#369b3d] text-white font-extrabold text-sm uppercase tracking-wider shadow-[0_0_20px_rgba(66,181,73,0.4)] transition-all whitespace-nowrap">
                {{ __('app.tokopedia.visit_btn') }} &rarr;
            </a>
        </div>
    </section>
    ...
    <section id="cues" class="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <div class="text-center max-w-3xl mx-auto mb-12">
            ...
            <p class="mt-4 text-slate-400 text-base sm:text-lg">
                {{ __('app.products.subtitle') }}
            </p>

            <!-- Category Filter Tabs -->
            <div class="mt-8 flex flex-wrap justify-center gap-2">
                <a href="{{ route('landing') }}#cues" class="px-5 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider border transition-all {{ request('category') == null || request('category') == 'all' ? 'bg-[#00e676] text-black border-[#00e676] shadow-[0_0_15px_rgba(0,230,118,0.4)]' : 'bg-[#121a15] text-slate-300 border-[#1f2e24] hover:border-[#00e676]/40' }}">
                    {{ __('app.products.all') }}
                </a>
                @foreach($categories as $cat)
                    <a href="{{ route('landing', ['category' => $cat->slug]) }}#cues" class="px-5 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider border transition-all {{ request('category') == $cat->slug ? 'bg-[#00e676] text-black border-[#00e676] shadow-[0_0_15px_rgba(0,230,118,0.4)]' : 'bg-[#121a15] text-slate-300 border-[#1f2e24] hover:border-[#00e676]/40' }}">
                        {{ $cat->name }}
                    </a>
                @endforeach
            </div>
                ...
                <div class="venom-card rounded-2xl overflow-hidden flex flex-col justify-between group">
                    <div>
                        ...
                        <div class="relative h-64 overflow-hidden bg-[#050806] flex items-center justify-center p-6 border-b border-[#1f2e24]">
                            ...
                            <img src="{{ $product->image_url }}" alt="{{ $product->name }}" class="h-full w-full object-cover transition-transform duration-500 group-hover:scale-110 filter drop-shadow-[0_0_10px_rgba(0,0,0,0.8)]">
                        </div>
                            ...
                            <div class="mt-4 grid grid-cols-2 gap-2 text-xs border-y border-[#1f2e24] py-3">
                                @if($product->tip_size)
                                    <div>
                                        ...
                                        <span class="text-slate-200 font-semibold">{{ $product->tip_size }}</span>
                                    </div>
                                @endif
                                @if($product->joint_type)
                                    <div>
                                        <span class="text-slate-500 block text-[10px] uppercase font-bold">Joint Pin</span>
                                        <span class="text-slate-200 font-semibold">{{ $product->joint_type }}</span>
                                    </div>
                                @endif
                                @if($product->chalk_friction)
                                    <div class="col-span-2">
                                        <span class="text-slate-500 block text-[10px] uppercase font-bold">Friction Matrix</span>
                                        <span class="text-[#00e676] font-semibold">{{ $product->chalk_friction }}</span>
                                    </div>
                                @endif
                            </div>
                        </div>
                    ...
                    <div class="p-6 pt-0 space-y-3">
                        <div class="flex items-center justify-between">
                            <div>
                                <span class="text-[10px] text-slate-400 block uppercase font-bold">{{ __('app.products.price_idr') }}</span>
                                <span class="text-xl font-black text-[#00e676]">{{ $product->formatted_price_idr }}</span>
                            </div>
                            <span class="px-2 py-0.5 rounded text-[10px] font-bold bg-[#10b981]/20 text-[#10b981] border border-[#10b981]/30">
                                {{ __('app.products.synced_with_tokopedia') }}
                            </span>
                        </div>

                        <div class="grid grid-cols-2 gap-2">
                            <!-- View Modal -->
                            <button @click="fetchDetail({{ $product->id }})" class="w-full py-2.5 rounded-lg bg-[#121a15] hover:bg-[#1f2e24] border border-[#1f2e24] text-slate-200 hover:text-white font-bold text-xs uppercase tracking-wider transition-colors">
                                {{ __('app.products.view_details') }}
                            </button>

                            <!-- Buy on Tokopedia -->
                            <a href="{{ $product->tokopedia_url ?: 'https://www.tokopedia.com/severus' }}" target="_blank" class="w-full py-2.5 rounded-lg bg-[#42b549] hover:bg-[#369b3d] text-white font-extrabold text-xs uppercase tracking-wider text-center flex items-center justify-center transition-colors">
    ...
    <section id="technology" class="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto border-t border-[#1f2e24]">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div class="venom-card p-8 rounded-2xl space-y-4">
                ...
                <h3 class="text-xl font-bold text-white">{{ __('app.tech.carbon_title') }}</h3>
                <p class="text-sm text-slate-400 leading-relaxed">{{ __('app.tech.carbon_desc') }}</p>
            </div>
            ...
            <div id="chalk" class="venom-card p-8 rounded-2xl space-y-4">
                ...
                <h3 class="text-xl font-bold text-white">{{ __('app.tech.chalk_title') }}</h3>
                <p class="text-sm text-slate-400 leading-relaxed">{{ __('app.tech.chalk_desc') }}</p>
            </div>
            ...
            <div id="accessories" class="venom-card p-8 rounded-2xl space-y-4">
                ...
                <h3 class="text-xl font-bold text-white">{{ __('app.tech.glove_title') }}</h3>
                <p class="text-sm text-slate-400 leading-relaxed">{{ __('app.tech.glove_desc') }}</p>
            </div>
        </div>
    </section>
                ...
                <div>
                    ...
                    <div class="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <span class="text-xs text-slate-400 uppercase font-bold block">{{ __('app.products.price_idr') }}</span>
                        ...
                        <div class="bg-[#121a15] p-4 rounded-xl border border-[#1f2e24]">
                            <h4 class="text-xs font-extrabold text-[#00e676] uppercase tracking-wider mb-3">{{ __('app.products.specifications') }}</h4>
                            <div class="grid grid-cols-2 gap-4 text-xs">
                                <div>
                                    <span class="text-slate-400 block font-medium">{{ __('app.products.tip_size') }}</span>
                                    <span class="text-white font-bold" x-text="activeProduct.tip_size"></span>
                                </div>
                                <div>
                                    <span class="text-slate-400 block font-medium">{{ __('app.products.joint_type') }}</span>
                                    <span class="text-white font-bold" x-text="activeProduct.joint_type"></span>
                                </div>
                                <div>
                                    <span class="text-slate-400 block font-medium">{{ __('app.products.weight') }}</span>
                                    <span class="text-white font-bold" x-text="activeProduct.weight_oz"></span>
                                </div>
                                <div>
                                    <span class="text-slate-400 block font-medium">{{ __('app.products.deflection') }}</span>
