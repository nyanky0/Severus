...
<div class="min-h-screen pt-28 pb-16 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between venom-card p-6 rounded-2xl">
        ...
        <a href="<?php echo e(route('admin.products.index')); ?>" class="px-4 py-2 rounded-xl bg-[#121a15] border border-[#1f2e24] text-slate-300 text-xs font-bold uppercase">
            &larr; Back to Catalog
        </a>
    </div>

    <form action="<?php echo e(route('admin.products.store')); ?>" method="POST" enctype="multipart/form-data" class="venom-card p-8 rounded-2xl space-y-6">
        <?php echo csrf_field(); ?>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
                ...
                <select name="category_id" required class="w-full px-4 py-3 rounded-xl bg-[#0a0f0d] border border-[#1f2e24] text-white text-sm">
                    <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($cat->id); ?>"><?php echo e($cat->name_en); ?> (<?php echo e($cat->name_id); ?>)</option>
            ...
            <div>
                ...
                <input type="number" step="1000" name="price_idr" value="<?php echo e(old('price_idr', 1500000)); ?>" required class="w-full px-4 py-3 rounded-xl bg-[#0a0f0d] border border-[#1f2e24] text-white text-sm">
            </div>

            <div>
                <label class="block text-xs font-bold text-slate-300 uppercase mb-2">English Product Name</label>
                <input type="text" name="name_en" required class="w-full px-4 py-3 rounded-xl bg-[#0a0f0d] border border-[#1f2e24] text-white text-sm" placeholder="e.g. Severus Carbon Viper Cue">
            </div>

            <div>
                <label class="block text-xs font-bold text-slate-300 uppercase mb-2">Indonesian Product Name</label>
                <input type="text" name="name_id" required class="w-full px-4 py-3 rounded-xl bg-[#0a0f0d] border border-[#1f2e24] text-white text-sm" placeholder="e.g. Stik Karbon Severus Viper">
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="image_url_input" class="w-full px-4 py-3 rounded-xl bg-[#0a0f0d] border border-[#1f2e24] text-white text-sm" placeholder="https://images.unsplash.com/...">
            </div>
        </div>
        ...
        <div class="border-t border-[#1f2e24] pt-6 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="text" name="tip_size" class="w-full px-3 py-2 rounded-lg bg-[#0a0f0d] border border-[#1f2e24] text-white text-xs" placeholder="e.g. 12.5mm Pro Taper">
                </div>

                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1">Joint Pin</label>
                    <input type="text" name="joint_type" class="w-full px-3 py-2 rounded-lg bg-[#0a0f0d] border border-[#1f2e24] text-white text-xs" placeholder="e.g. Radial Joint">
                </div>

                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1">Weight</label>
                    <input type="text" name="weight_oz" class="w-full px-3 py-2 rounded-lg bg-[#0a0f0d] border border-[#1f2e24] text-white text-xs" placeholder="e.g. 19.0 oz">
                </div>

                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1">Deflection Grade</label>
                    <input type="text" name="deflection_grade" class="w-full px-3 py-2 rounded-lg bg-[#0a0f0d] border border-[#1f2e24] text-white text-xs" placeholder="e.g. Ultra Low Deflection">
                </div>

                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1">Chalk Friction Matrix</label>
                    <input type="text" name="chalk_friction" class="w-full px-3 py-2 rounded-lg bg-[#0a0f0d] border border-[#1f2e24] text-white text-xs" placeholder="e.g. Grade 9.9 Nano-Grip">
                </div>

                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase mb-1">Available Stock</label>
                    <input type="number" name="stock" value="10" class="w-full px-3 py-2 rounded-lg bg-[#0a0f0d] border border-[#1f2e24] text-white text-xs">
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
                ...
                <textarea name="description_en" rows="3" required class="w-full px-4 py-3 rounded-xl bg-[#0a0f0d] border border-[#1f2e24] text-white text-sm"></textarea>
            </div>

            <div>
                <label class="block text-xs font-bold text-slate-300 uppercase mb-2">Indonesian Description</label>
                <textarea name="description_id" rows="3" required class="w-full px-4 py-3 rounded-xl bg-[#0a0f0d] border border-[#1f2e24] text-white text-sm"></textarea>
            </div>
        </div>

        <div class="flex items-center space-x-6">
            <label class="flex items-center space-x-2 text-xs text-white font-bold">
                <input type="checkbox" name="is_featured" checked class="rounded bg-[#0a0f0d] border-[#1f2e24] text-[#00e676]">
                <span>Featured Product (Homepage Showcase)</span>
            </label>

            <label class="flex items-center space-x-2 text-xs text-white font-bold">
                <input type="checkbox" name="is_active" checked class="rounded bg-[#0a0f0d] border-[#1f2e24] text-[#00e676]">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/html/resources/views/admin/products/create.blade.php ENDPATH**/ ?>